...
import { seedDesdeEtiquetado } from "../lib/nexusModel";
...
function Toggle({ pregunta, descripcion, valor, onChange, emoji }) {
  ...
  return (
    <div style={{
      background: t.bgCard,
      border: `1px solid ${valor ? t.accent : t.border}`,
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
        ...
        <button onClick={() => onChange(!valor)} style={{
          ...
          background: valor ? t.accent : t.border,
...
export default function Calibracion({ onIniciarSesion }) {
  ...
  const [usaSegundoMonitor, setUsaSegundoMonitor] = useState(false);
  const [usaLecturaFisica,  setUsaLecturaFisica]  = useState(false);
  ...
  const [poses, setPoses] = useState({
    ...
    monitor2:  { done: false, cuenta: 0, capturando: false, progreso: 0 },
    lectura:   { done: false, cuenta: 0, capturando: false, progreso: 0 },
  });
  ...
  function finalizarCalibracion() {
    localStorage.setItem("nexus_perfil_calibracion", JSON.stringify({
      habla, videollamada, usaMovil, fecha: new Date().toISOString(),
    }));

    localStorage.setItem("nexus_vectores_calibrados", JSON.stringify(vectores.current));
  ...
  const posesRequeridas = [
    "neutral", "distraido", "ausente",
    ...(usaMovil ? [] : ["manos"]),
    ...(usaSegundoMonitor ? ["monitor2"] : []),
    ...(usaLecturaFisica ? ["lectura"] : []),
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{ 
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <img src="/assets/blue_logo.png" alt="Nexus Logo" style={{ width: 44, height: 44, objectFit: "contain", filter: "hue-rotate(80deg) saturate(1.6) brightness(0.95) contrast(1.05) drop-shadow(0 4px 10px rgba(191,90,242,0.35))" }} />
          <div>
            <span style={{ color: t.text, fontWeight: 800, fontSize: 24, display: "block", lineHeight: 1 }}>Nexus</span>
      ...
      <div style={{ width: "100%", maxWidth: 560 }}>
        ...
        {paso === "cuestionario" && (
          <>
            ...
            <div style={{ 
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input
                  ...
                  onFocus={(e) => e.target.style.border = `2px solid ${t.accent}`}
            ...
            <Toggle emoji="🖥️" pregunta="¿Usas un segundo monitor mientras estudias?"
              descripcion="Calibraremos esa posición como concentración, no como distracción."
              valor={usaSegundoMonitor} onChange={setUsaSegundoMonitor} />
            <Toggle emoji="📖" pregunta="¿Sueles consultar apuntes o libros en papel?"
              descripcion="Calibraremos mirar hacia abajo como concentración, no como distracción."
              valor={usaLecturaFisica} onChange={setUsaLecturaFisica} />
            ...
            <p style={{ color: t.textLight, fontSize: 11, textAlign: "center", margin: "6px 0 0" }}>
              Nexus usará detección básica sin personalización
            </p>
          </>
        )}
        ...
        {paso === "camara" && (
          <>
            ...
            {usaSegundoMonitor && (
              <PasoCal titulo="Posición segundo monitor — Concentrado"
                instruccion="Mira tu monitor secundario como si estuvieras trabajando en él."
                emoji="🖥️" color={t.green} {...poses.monitor2}
                deshabilitado={!poses.ausente.done}
                onIniciar={() => capturarPose("monitor2", "concentrado")} />
            )}
            {usaLecturaFisica && (
              <PasoCal titulo="Posición lectura — Concentrado"
                instruccion="Mira hacia abajo como si estuvieras leyendo un libro o tus apuntes."
                emoji="📖" color={t.green} {...poses.lectura}
                deshabilitado={!poses.ausente.done}
                onIniciar={() => capturarPose("lectura", "concentrado")} />
            )}
