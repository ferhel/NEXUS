...
export default function NotasRapidas({ sesionId }) {
  ...
  const [notas, setNotas] = useState(() => {
    const guardadas = localStorage.getItem(`nexus_notas_${sesionId}`);
    return guardadas ? JSON.parse(guardadas) : [{ id: Date.now(), texto: "", color: COLORES[0] }];
  });

  // --- ESTADO DE TAREAS (Micro-objetivos) ---
  const [tareas, setTareas] = useState(() => {
    const guardadas = localStorage.getItem(`nexus_tareas_${sesionId}`);
  ...
  useEffect(() => {
    localStorage.setItem(`nexus_notas_${sesionId}`, JSON.stringify(notas));
    localStorage.setItem(`nexus_tareas_${sesionId}`, JSON.stringify(tareas));
    ...
    <div style={{ fontFamily: font, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ background: t.bgCard, borderRadius: 20, padding: 20, border: `1px solid ${t.border}`, boxShadow: t.shadow }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <span style={{ color: t.accent, fontSize: 12, fontWeight: 800 }}>{porcentaje}%</span>
        </div>

        {/* Barra de Progreso de Tareas */}
        <div style={{ height: 6, background: t.bgMuted, borderRadius: 3, marginBottom: 20, overflow: "hidden" }}>
          <div style={{ width: `${porcentaje}%`, height: "100%", background: t.accent, transition: "width 0.4s ease" }} />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {tareas.map(tarea => (
            <div key={tarea.id} style={{ 
              ...
              background: tarea.completada ? "transparent" : t.bgMuted,
              ...
              <input type="checkbox" checked={tarea.completada} onChange={() => toggleTarea(tarea.id)} 
                style={{ cursor: "pointer", accentColor: t.accent, width: 16, height: 16 }} />
              <span style={{ 
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={agregarNota} style={{ background: t.accent, color: "#fff", border: "none", borderRadius: "50%", width: 24, height: 24, cursor: "pointer", fontWeight: 700 }}>+</button>
        </div>
