...
const ETIQUETAS_LABEL = { concentrado: "Concentrado/a", distraido: "Distraído/a", ausente: "Ausente" };
const ETIQUETAS_COLOR = { concentrado: "green", distraido: "amber", ausente: "red" };
    ...
    celdas.push(
      <div key={`dia-${dia}`} style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
        <div style={{
          ...
          background: estudio ? t.accent : (esHoy ? t.bgMuted : "transparent"),
          color: estudio ? "#fff" : (esHoy ? t.accent : t.textLight),
          boxShadow: estudio ? "0 2px 8px rgba(191, 90, 242, 0.3)" : "none",
    ...
    <div style={{ background: t.bgCard, borderRadius: 24, padding: "28px", boxShadow: t.shadowMd, border: `1px solid ${t.border}`, marginTop: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          ...
          <span style={{ color: t.accent, fontSize: 13, fontWeight: 800, background: "rgba(191, 90, 242, 0.16)", padding: "6px 14px", borderRadius: 12, border: `1px solid rgba(191, 90, 242, 0.3)` }}>
...
export default function FeedbackSesion({ reporte, onContinuar }) {
  ...
  const [patronesNuevos, setPatronesNuevos] = useState([]);
  const [patronAprendido, setPatronAprendido] = useState(null);

  useEffect(() => {
    const guardado = JSON.parse(localStorage.getItem("nexus_historial") || "[]");
    setHistorialPrevio(guardado);
    setPatronesNuevos(reporte?.patronesNuevos || []);

    if (reporte?.sesionId) {
      const guardadasNotas = localStorage.getItem(`nexus_notas_${reporte.sesionId}`);
      if (guardadasNotas) {
        setNotas(JSON.parse(guardadasNotas).filter(n => n.texto && n.texto.trim() !== ""));
      }

      const guardadasTareas = localStorage.getItem(`nexus_tareas_${reporte.sesionId}`);
      if (guardadasTareas) {
  ...
  const handleEtiquetarPatron = (idx, etiquetaElegida) => {
    const patron = patronesNuevos[idx];
    if (!patron) return;
    reentrenar(patron.vectoresMuestra.map(vector => ({ vector, etiqueta: etiquetaElegida })));
    setPatronesNuevos(prev => prev.filter((_, i) => i !== idx));
    setPatronAprendido(ETIQUETAS_LABEL[etiquetaElegida] || etiquetaElegida);
  };

  const handleGuardarYContinuar = () => {
    try {
      if (reporte) {
        const hPrevio = JSON.parse(localStorage.getItem("nexus_historial") || "[]");
        const nuevaSesion = {
          ...reporte,
        ...
        localStorage.setItem("nexus_historial", JSON.stringify(hPrevio));
      }
    } catch (error) {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 32, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            ...
            <StatCard label="MICRO-OBJETIVOS" value={totalTareas > 0 ? `${tareasCompletadas}/${totalTareas}` : "0/0"} color={t.accent} />
          ...
          <div style={{ background: t.bgCard, borderRadius: 24, padding: "28px", boxShadow: t.shadowMd, border: `1px solid ${t.border}` }}>
            ...
            <div style={{ height: 12, width: "100%", borderRadius: 6, display: "flex", overflow: "hidden", marginBottom: 20, background: t.bgMuted }}>
              <div style={{ width: `${ipvGlobalLimpio}%`, background: t.accent, transition: "width 0.5s ease" }} />
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <TimeMetric dotColor={t.accent} label="En Foco" time={formatSegundos(tConcentrado)} />
            ...
            <div style={{ width: "100%", height: 140 }}>
              {len > 0 ? (
                <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" style={{ overflow: "visible" }}>
                  ...
                  {len > 1 && (
                    <path d={pathCurvo} fill="none" stroke={t.accent} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" style={{ filter: "drop-shadow(0px 4px 6px rgba(191, 90, 242, 0.3))" }} />
                  )}
                  {puntos.map((p, i) => (
                    <g key={i}>
                      <circle cx={p.x} cy={p.y} r="5" fill={t.bgCard} stroke={t.accent} strokeWidth="3" />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 20, padding: "24px", boxShadow: t.shadow }}>
             <p style={{ color: t.purple, fontSize: 12, fontWeight: 800, letterSpacing: 1, marginBottom: 12 }}>ANÁLISIS DE NEXUS AI</p>
             <p style={{ fontSize: 15, color: t.text, margin: 0, lineHeight: 1.6, fontWeight: 500 }}>{diagnosticoIA}</p>
          </div>

          {patronesNuevos.length > 0 && (
            <div style={{ background: t.bgCard, border: `1px solid ${t.accent}`, borderRadius: 20, padding: "24px", boxShadow: t.shadow }}>
              <p style={{ color: t.accent, fontSize: 12, fontWeight: 800, letterSpacing: 1, marginBottom: 8 }}>🧭 POSTURA NO RECONOCIDA</p>
              <p style={{ fontSize: 13, color: t.textMuted, margin: "0 0 16px", lineHeight: 1.5 }}>
                Nexus detectó una posición que tu calibración no cubría (ej. otro monitor, apuntes en papel). Ayúdanos a etiquetarla para mejorar la próxima detección.
              </p>
              {patronesNuevos.map((patron, idx) => {
                const segundos = Math.round(patron.tamano * (INTERVALO_MS / 1000));
                return (
                  <div key={idx} style={{ background: t.bgMuted, borderRadius: 14, padding: "14px 16px", marginBottom: 10 }}>
                    <p style={{ fontSize: 13, color: t.text, margin: "0 0 10px", lineHeight: 1.5 }}>
                      Postura sostenida ~{formatSegundos(segundos)}, hoy clasificada como "{ETIQUETAS_LABEL[patron.etiquetaSugerida] || patron.etiquetaSugerida}". ¿Qué era en realidad?
                    </p>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {Object.keys(ETIQUETAS_LABEL).map(etiqueta => (
                        <button key={etiqueta} onClick={() => handleEtiquetarPatron(idx, etiqueta)} style={{
                          padding: "7px 14px", borderRadius: 10, cursor: "pointer", fontWeight: 600, fontSize: 12,
                          background: "transparent", border: `1px solid ${t[ETIQUETAS_COLOR[etiqueta]]}`, color: t[ETIQUETAS_COLOR[etiqueta]],
                        }}>
                          {ETIQUETAS_LABEL[etiqueta]}
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          {patronAprendido && (
            <p style={{ color: t.green, fontSize: 12, fontWeight: 700, textAlign: "center", margin: 0 }}>
              ✓ Nexus aprendió esta postura como "{patronAprendido}"
            </p>
          )}
      ...
      <button onClick={handleGuardarYContinuar} style={{
        ...
        background: t.accent, color: "#fff", fontWeight: 700, fontSize: 16, letterSpacing: 1,
