...
// ─────────────────────────────────────────────────────────────
function getIpvColor(ipv, t) {
  return ipv >= 80 ? t.green : ipv >= 50 ? t.amber : t.red;
}

function getIpvBg(ipv) {
  return ipv >= 80 ? "rgba(48, 209, 88, 0.15)" : ipv >= 50 ? "rgba(255, 159, 10, 0.15)" : "rgba(255, 69, 58, 0.15)";
}
    ...
    <div style={{
      ...
      <div style={{ width: "100%", height: H }}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" style={{ overflow: "visible" }}>
          ...
          <polyline points={polylinePoints} fill="none" stroke={t.accent} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          ...
          {datos.map((p, i) => {
            ...
            return (
              <g key={i}>
                <circle cx={x} cy={y} r="4" fill={t.bgCard} stroke={t.accent} strokeWidth="2.5" />
...
function TarjetaMateria({ materia }) {
  ...
  const color = getIpvColor(promedioIpv, t);
  ...
  if (promedioIpv >= 85) {
    ...
    insight = "👀 Alta dispersión. Esta área exige mucho costo ejecutivo. Nexus sugiere usar Ruido Marrón y acortar los Pomodoros a 15-20 min.";
    ...
    <div style={{
      ...
      <div style={{ background: t.bgMuted, padding: "12px 16px", borderRadius: 12, borderLeft: `3px solid ${color}` }}>
        <p style={{ margin: 0, fontSize: 13, color: t.text, lineHeight: 1.5 }}>
          <span style={{ fontWeight: 700, color: color }}>Análisis Nexus: </span>
          {insight}
        </p>
      </div>
    </div>
...
function TarjetaSesion({ sesion, onEliminar }) {
  ...
  const color = getIpvColor(sesion.ipvGlobal || 0, t);
...
export default function Historial() {
  ...
  useEffect(() => {
    try {
      const data = JSON.parse(localStorage.getItem("nexus_historial") || "[]");
      setSesiones(data);
  ...
  function eliminar(idx) {
    if(window.confirm("¿Segura que deseas eliminar esta sesión? Afectará tus estadísticas.")) {
      ...
      setSesiones(nuevas);
      localStorage.setItem("nexus_historial", JSON.stringify(nuevas));
  ...
  function limpiarTodo() {
    if (window.confirm("⚠️ ¿Peligro: Eliminar todo el historial y reiniciar métricas?")) {
      ...
      localStorage.removeItem("nexus_historial");
    }
  }
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 600, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src="/assets/blue_logo.png" alt="Nexus" style={{ width: 32, height: 32, objectFit: "contain", filter: "hue-rotate(80deg) saturate(1.6) brightness(0.95) contrast(1.05)" }} />
          <span style={{ color: t.text, fontWeight: 800, fontSize: 20 }}>Nexus</span>
          <span style={{ color: t.textMuted, fontSize: 14, fontWeight: 600 }}>/ Intelligence</span>
        </div>
        <button onClick={() => navigate("/")} style={{
          background: t.accent, color: "#fff", border: "none", borderRadius: 12, padding: "8px 16px",
      ...
      <div style={{ width: "100%", maxWidth: 600 }}>
        ...
        {totalSesiones > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
            ...
            <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 16, padding: "16px", textAlign: "center", boxShadow: t.shadow }}>
              <div style={{ color: getIpvColor(ipvPromedio, t), fontWeight: 800, fontSize: 24, marginBottom: 4 }}>{ipvPromedio}%</div>
              <div style={{ color: t.textMuted, fontSize: 10, fontWeight: 700, letterSpacing: 1 }}>IPV PROMEDIO</div>
            </div>
            <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 16, padding: "16px", textAlign: "center", boxShadow: t.shadow }}>
              <div style={{ color: t.accent, fontWeight: 800, fontSize: 24, marginBottom: 4 }}>{formatMinutosAHoras(totalMinutos)}</div>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 24, borderBottom: `2px solid ${t.borderMid}` }}>
            <button 
              ...
              style={{ 
                ...
                color: vista === "materias" ? t.accent : t.textMuted,
                borderBottom: vista === "materias" ? `3px solid ${t.accent}` : "3px solid transparent",
            ...
            <button 
              ...
              style={{ 
                background: "none", border: "none", padding: "12px 0", cursor: "pointer", fontSize: 14, fontWeight: 700,
                color: vista === "lista" ? t.accent : t.textMuted,
                borderBottom: vista === "lista" ? `3px solid ${t.accent}` : "3px solid transparent",
                transform: "translateY(2px)"
              }}>
              Sesiones Recientes
            </button>
        ...
        {sesiones.length === 0 && (
          <div style={{ textAlign: "center", padding: "56px 20px", background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 24, boxShadow: t.shadow }}>
            ...
            <p style={{ color: t.textMuted, fontSize: 14, margin: "0 0 24px", lineHeight: 1.5 }}>
              Nexus necesita datos. Haz tu primera sesión de estudio para comenzar a generar métricas predictivas.
            </p>
            <button onClick={() => navigate("/calibracion")} style={{
              padding: "14px 28px", borderRadius: 16, background: t.accent, color: "#fff",
              fontWeight: 700, fontSize: 14, border: "none", cursor: "pointer", boxShadow: "0 4px 12px rgba(191, 90, 242, 0.3)"
            }}>
        ...
        {sesiones.length > 0 && vista === "materias" && (
          <div>
            <p style={{ color: t.textLight, fontSize: 13, marginBottom: 16 }}>
              Nexus agrupa automáticamente tus sesiones por materia y analiza tu comportamiento para darte recomendaciones específicas.
            </p>
