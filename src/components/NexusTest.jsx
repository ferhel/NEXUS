...
// ─────────────────────────────────────────────────────────────
// ESTILOS BASE (dependen del tema activo, recalculados en cada
// componente para que reaccionen al toggle claro/oscuro)
// ─────────────────────────────────────────────────────────────
function crearEstilosBase(theme) {
  return {
    containerStyle: { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: theme.bg, padding: 20 },
    cardStyle: { width: "100%", background: theme.bgCard, padding: "40px", borderRadius: 32, boxShadow: theme.shadowMd, border: `1px solid ${theme.border}`, textAlign: "center" },
    logoIconStyle: { width: 70, height: 70, background: "rgba(191, 90, 242, 0.10)", borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 28px" },
    btnPrincipal: { width: "100%", padding: "16px", borderRadius: 14, border: "none", background: theme.accent, color: "#fff", fontWeight: 700, fontSize: 16, cursor: "pointer" },
    btnGhost: { padding: 10, background: "none", border: "none", color: theme.textMuted, fontWeight: 700, cursor: "pointer", fontSize: 14 },
    bipolarTextStyle: { flex: 1, fontSize: 13, fontWeight: 600, color: theme.text, lineHeight: 1.3 },
    radarContainerStyle: { background: theme.bgCard, padding: 32, borderRadius: 28, border: `1px solid ${theme.border}`, display: "flex", justifyContent: "center", marginBottom: 24 },
    resCardBase: { background: theme.bgCard, padding: 28, borderRadius: 24, border: `1px solid ${theme.border}` },
    hStyle: { fontSize: 11, fontWeight: 900, letterSpacing: 2, marginBottom: 18, textTransform: "uppercase", color: theme.accent },
    ulStyle: { paddingLeft: 18, fontSize: 14, color: theme.text, lineHeight: 1.7, margin: 0 },
    iaAnalisisBox: { background: theme.bgMuted, padding: 18, borderRadius: 16, marginTop: 20 },
  };
}
...
function PantallaInicio({ onStart }) {
  ...
  const S = crearEstilosBase(theme);
  return (
    <div style={S.containerStyle}>
      <div style={{ ...S.cardStyle, maxWidth: 440, padding: "48px 40px" }}>
        <div style={S.logoIconStyle}>
          <img src="/assets/blue_logo.png" alt="Nexus" style={{ width: 42, filter: "hue-rotate(80deg) saturate(1.6) brightness(0.95) contrast(1.05)" }} />
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-0.5px", marginBottom: 12, color: theme.text }}>Hola, soy Nexus.</h1>
        ...
        <button onClick={onStart} style={S.btnPrincipal}>Empezar análisis</button>
      </div>
    </div>
  );
}
...
function Cuestionario({ q, idx, total, valor, onRes, onNext, onPrev }) {
  ...
  return (
    <div style={S.containerStyle}>
      <div style={{ ...S.cardStyle, maxWidth: 540 }}>
        <BarraProgreso actual={idx + 1} total={total} />

        <h2 style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.4, minHeight: 90, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 32, padding: "0 10px", color: theme.text }}>
          {q?.texto}
        </h2>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 44, alignItems: "center" }}>
          <button onClick={onPrev} disabled={idx === 0} style={S.btnGhost}>Atrás</button>
          <button onClick={onNext} disabled={!valor} style={{ ...S.btnPrincipal, width: "auto", padding: "12px 36px", borderRadius: 14, fontSize: 15 }}>
...
function CuestionarioBipolar({ q, idx, valor, onRes, onNext }) {
  ...
  return (
    <div style={S.containerStyle}>
      <div style={{ ...S.cardStyle, maxWidth: 640 }}>
        <p style={{ fontSize: 11, fontWeight: 800, color: theme.accent, letterSpacing: 2, marginBottom: 24, textTransform: "uppercase" }}>Tendencia Cognitiva</p>
        <h2 style={{ fontSize: 19, fontWeight: 700, marginBottom: 40, lineHeight: 1.4, color: theme.text }}>¿Con qué estilo te sientes más cómoda?</h2>

        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20, padding: "0 15px" }}>
          <span style={S.bipolarTextStyle}>{q.textoA}</span>
          <span style={{ ...S.bipolarTextStyle, textAlign: "right" }}>{q.textoB}</span>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 8 }}>
          {[1, 2, 3, 4, 5, 6, 7].map(n => (
            <button key={n} onClick={() => onRes(n)} style={{
              ...
              background: valor === n ? theme.accent : "transparent", color: valor === n ? "#fff" : theme.text,
              fontWeight: 700, fontSize: 15, cursor: "pointer", transition: "0.2s"
            }}>{n}</button>
          ))}
        </div>

        <button onClick={onNext} disabled={!valor} style={{ ...S.btnPrincipal, marginTop: 44 }}>Generar Perfil</button>
      </div>
    </div>
  );
}
    ...
    <div style={{ background: theme.bg, minHeight: "100vh", padding: "60px 20px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <p style={{ color: theme.accent, fontWeight: 800, letterSpacing: 3, fontSize: 11, marginBottom: 8 }}>IDENTIDAD COGNITIVA NEXUS</p>
          <h1 style={{ fontSize: 40, fontWeight: 900, letterSpacing: "-1px", margin: 0, color: theme.text }}>
        ...
        <div style={S.radarContainerStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 24 }}>
          ...
          <div style={S.resCardBase}>
            <h4 style={S.hStyle}>🧠 Bases Pedagógicas</h4>
            <p style={{ fontSize: 14, lineHeight: 1.6, color: theme.text }}>{p.baseCientifica}</p>
            <div style={S.iaAnalisisBox}>
              <p style={{ fontSize: 11, fontWeight: 900, color: theme.accent, marginBottom: 6 }}>NEXUS AI INSIGHT</p>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} onClick={() => onChange(n)} style={{
          padding: "16px 4px", borderRadius: 14, border: valor === n ? `2px solid ${theme.accent}` : `1.5px solid ${theme.border}`,
          background: valor === n ? "rgba(191, 90, 242, 0.14)" : "transparent", transition: "0.2s"
        }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: valor === n ? theme.accent : theme.text }}>{n}</div>
...
function RadarGrafico({ dimensiones }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <polygon points={path} fill="rgba(191, 90, 242, 0.18)" stroke={theme.accent} strokeWidth="2" />
    </svg>
  );
}
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      ...
      <div style={{ height: 5, background: theme.border, borderRadius: 10, overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: theme.accent, transition: "0.5s" }} />
      </div>
    </div>
  );
}
...
function ResultCard({ title, list, highlight }) {
  ...
  const S = crearEstilosBase(theme);
  return (
    <div style={{ ...S.resCardBase, border: highlight ? `1px solid ${theme.accent}` : `1px solid ${theme.border}`, background: highlight ? "rgba(191, 90, 242, 0.06)" : theme.bgCard }}>
      <h4 style={S.hStyle}>{title}</h4>
      <ul style={S.ulStyle}>{list.map((item, i) => <li key={i}>{item}</li>)}</ul>
    </div>
  );
}
